...
// save original text palette
{
 ...
 _dos_creat( "editor.pal", _A_NORMAL, &handle);
  _dos_write(handle, Palette, 256*3, &sbytes); //_dos_write(handle, Palette, 16*3, &sbytes);
 _dos_close(handle);
}
...
// load original text palette
{
 ...
 _dos_open( "editor.pal", O_RDONLY, &handle);
  _dos_read(handle, Palette, 256*3, &sbytes); //_dos_read(handle, Palette, 16*3, &sbytes);
 _dos_close(handle);

 GFXSetPal( 255 ); //GFXSetPal( 15 );
}
...
// - use GFXSetPal( 255 ) to apply palette
{
 int  handle;
 word sbytes;

 _dos_open( palFile, O_RDONLY, &handle);
...
// - saves pallete to file
{
 int  handle;
 word sbytes;

 _dos_creat( palFile, _A_NORMAL, &handle);
